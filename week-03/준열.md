...
- 업데이터 함수(setCount(c => c + 1))를 사용하여 의존성에서 해당 상태를 제거할 수 있다.
...
- 특정 값의 변화에 **`반응`** 하고 싶지는 않지만, 로직 내에서 **`최신 값`** 은 참조해야 할 때가 있다.

- **`useEffectEvent`** 를 사용하여 비반응형 로직을 분리하면, 해당 값의 변화로 인한 재실행 없이 최신 상태를 읽을 수 있다.

### Displaying different content on the server and the client

- 서버와 클라이언트의 초기 HTML이 다르면 **`Hydration`** 에러가 발생한다. ex) localStorage 사용 시)

- **`didMount`** 상태를 사용하여 클라이언트에서 마운트된 후에만 특정 UI를 보여주도록 처리한다.
